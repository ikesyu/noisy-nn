...
*L=32 は配置適合するが、72% 充填 + `--freq 75` のタイミング駆動配線が
オープンフローのルータで振動し実質収束しない（17 時間で残アーク 116.9k →
113.6k）。`--freq 50` での再配線結果は下記。**確定推奨構成は L=16**:
更新 ~590 Hz はロボット制御周期（100 Hz–1 kHz）に足り、192k 提示の
オンチップ学習は ~5.4 分。COMB 残余 45% は Phase 4 の通信ラッパ等に使える。
