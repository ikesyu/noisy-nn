...
> [new idea — **有界ノイズ（一様＝コンパクト台）は入力ごとの厳密な活動ゲートを与える**（`idea_core.md` §2.7.3–§2.7.4）。
> 上の撤回は「$\sigma$ を縮めても電力は落ちない（ゲージ依存）」だが、それとは別軸の合法な電力削減がカーネルの台にある。
> 本計画は既に一様ノイズ（放物線活性、LFSR、§1・§4）を使っており、その標準化前活性 $|s_k|=|d_k-h_k|/\sigma_k$ が
> **台半径を超えた入力では交差が厳密に 0**（ガウスは漸近のみ）。ゆえに**その入力・そのユニットに限り**比較器＋巡回 XOR ＋
> カウンタをクロックゲートしてよい——$\nu_k$ 平均ではなく**入力ごとの $s_k$ で判定する動的・イベント駆動の電力ゲート**であり、
> 出力はビット単位で不変（近似ではない）。$\nu_k=0$ の静的 kill（恒久退役）とは直交する第 2 の節電軸。実装上は
> $d_k$ が台の外か（$|d_k-h_k|\ge r_u$）を 1 比較で判定して当該 PE レーンを休止させるだけ。Phase 2/3 の CrossingEngine で
> 「台外レーンのクロックゲート率」を実測する価値がある（データ分布依存だが、飽和領域に入る入力ほど得をする）。]
...
時点でペース 1/3 に低下）で完走見込みが立たず、8 日目にルータが segfault で
異常終了（2026-08-05）。オープンフローでの L=32 配線は打ち切りで確定。商用ツール
