...
- **(c) 連続制御の定量化（L4）【実施済み、2026-08-04、結果は §7.11】**: 質的部分（単調・連続な制御軸、
  離散スイッチの否定）は 3 seed で決定的に成立。量的部分（中間行動の精度）は不成立で、原稿では
  主張しない。残る枝は §7.11 末尾の 3 つ（中間場を学習に含める／$\alpha_\text{mix}$ 依存／行動レベル）で、
  いずれも優先度は高くない。
...
- **(g) セクター知覚の行動品質【ほぼ解決、2026-08-07、結果は §7.12b】**: 原因は操舵精度でも
  容量でもなく「純粋場・固定シーンでしか訓練していない」ことだった。実行時分布での学習
  （`protocol.train_blended`：混合 $w$ × threat 乱配置 × ランダム位置）で 3 症状は解消し
  行動指標は vector 同等。残差は壁帯域滞在 14.4%（教師オラクルのフロア 8.6%）のみ。
  SR 系スクリプト（§7.1 ほか）の sector 移行はまだ（vector のまま）。
- **(h) 入力共有の定量指標（§7.12）**: $\nu$ サポートの Jaccard は場の幾何だけで決まり
  符号化に不感。第 1 層実効重みの行動間共有度など、入力基盤の共有を直接測る指標を設計する。
- **(i) 調停のモジュレータ ODE 化（Stage 2、§7.12a）**: goal ビット・refuge・risk・概日的
  長周期切替を、3 モジュレータの漏れ積分器＋相互作用に統合する。
