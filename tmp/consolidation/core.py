"""consolidation.core — noise-field self-consolidation primitives
(docs/idea_consolidation.md). (Formerly tmp/consolidation_lib.py; now the
tmp/consolidation/ package. `import consolidation` re-exports this module;
`consolidation_lib` remains as a compatibility shim.)
...
PROJECT_ROOT = Path(__file__).resolve().parents[2]  # tmp/consolidation/core.py -> repo root
