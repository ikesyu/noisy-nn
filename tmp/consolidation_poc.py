...
forward-only で完結する。アニールとのインターリーブにはミラー・オプティマイザ
状態の永続が要るため、fncl_driver.train_cov の cov_jac 分岐を永続状態の
CovJacTrainer として再実装している（数式は train_cov と同一）。
...
class ConsolidableNNN(model.SimpleNNNSample):
    """SimpleNNNSample + 層ごとの per-unit ノイズベクトル `sigma_vecs`.

    forward で stds を渡さなければ sigma_vecs（[H] テンソルのリスト）が使われ、
    GaussianNoiseLayer 内で [N, T, H] にブロードキャストされる。交差しきい値 h は
    スカラーのまま層に凍結されているので、sigma_vecs の操作は idea_consolidation.md
    §4.2 の消滅経路 (a)（h 固定・σ のみ縮小）に厳密に対応する。
    """
    ...
    def forward(self, x: torch.Tensor, stds: list = None) -> torch.Tensor:
        ...
        if getattr(self, "h_vecs", None) is not None:
            for i, gc in enumerate(self.gaussian_crossing):
                gc.h = self.h_vecs[i]          # [H]; Crossing 内でブロードキャスト
...
H_DEAD = 1.0e6      # スナップ後のしきい値番兵: 上流ゆらぎでも交差不能


def kill_unit(net, l: int, k: int) -> None:
    """ユニット (l, k) の厳密な離脱。

    σ_k = 0 は注入ノイズを止めるだけで、深い層 (l >= 1) では上流のサンプル
    ゆらぎが交差を駆動し続ける（σ=0 でも発火する）。しきい値 h_k を有限の
    大きな値に上げると、有界な上流ゆらぎでは交差できず z_k = 0 が厳密に
    成立し、KDE スロープと credit も 0 になって次層列の再成長も起こらない。
    """
...
# ============================================================
# cov_jac の永続状態トレーナ
# （fncl_driver.train_cov の method=="cov_jac", jac_track=True, slope=kde,
#   credit 不使用（cov_jac は共分散ミラー + 厳密出力誤差のみ）と同一の数式。
#   アニールと 1 epoch 単位で交互実行するため、ミラー W とオプティマイザ状態を
#   呼び出しをまたいで保持する。）
# ============================================================
class CovJacTrainer:
    ...
    def step(self) -> float:
        ...
        with torch.no_grad():
            ...
            self.W["out"] = self.W["out"] - steps["out"]     # Kolen-Pollack 追跡
            for l in range(1, n_hidden):
...
def collect_stats(net, x, passes: int = 4):
    """per-sample 活性 z[l] [N, P*T, H] と期待活性 zbar[l] [N, H]."""
    cap = Capture(net)
...
def ridge_fit(Zr: torch.Tensor, zk: torch.Tensor, lam: float):
    """中心化リッジ回帰 zk ~ Zr a + c。Zr [N, R], zk [N] -> (a [R], c, resid [N])."""
...
def run_route_B(net, x, target, pred0, quota: dict, tol: float, args,
    ...
    while any(quota[l] > 0 for l in quota):
        ...
        if random_order:
            ...
            _, l, k = best
        # --- 消滅経路のアニール + 閉ループ学習 (§7.1/§7.7) ---
        # 層1 (l=0): σ_k のみ下げる（上流が決定的なので σ→0 で厳密フェード）。
        # 深層 (l>=1): σ_k を下げると同時に h_k を上げる。σ=0 は注入ノイズを
        # 止めるだけで上流サンプルゆらぎによる交差が残るため、しきい値
        # エスカレーションが本来の消滅ノブになる（(σ, h) 平面の垂直経路）。
        steps = 0
        while True:
            ...
            rec("train", net, trainer, removed, (l, k))
            holds = 0
            while trainer.ema() > tol and holds < args.max_holds:
                trainer.run(args.epochs_per_step)      # ドリフト超過: アニール保留
                rec("hold", net, trainer, removed, (l, k))
                holds += 1
            holds_total += holds
            steps += 1
            act = float(trainer.cap.z[l][:, :, k].mean())   # 実測平均活動 (§4.4)
