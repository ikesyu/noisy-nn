"""
fncl5_7.py — 論文 §5.7「読み出し誤差も共分散から — cov_jac_full と歪度バイアス」
...
(a) 読み出し誤差推定量の比較 (Adam): cov_jac_full の --jac-out 3 変種
    ...
    --include-sgd で SGD 変種も追加 (推定量によらず cov_jac_sgd の最適化床に
...
実行例:
  python tmp/fncl5_7.py                # 既定: H=64, T=64, 1500 epochs, seeds 0,1,2
  python tmp/fncl5_7.py --include-sgd
  python tmp/fncl5_7.py --quick
"""
import argparse
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="§5.7 readout error from covariance (cov_jac_full): "
                    "skewness bias, its Adam-amplified drift, and the fixes.")
    add_common_args(p)
    p.add_argument("--noise", choices=("gaussian", "uniform"), default="gaussian")
    p.add_argument("--include-sgd", action="store_true",
                   help="SGD 変種 3 種も行列に追加する")
    ...
    args = finalize_args(p.parse_args(), "out/fncl5_7")
    if args.quick:
    ...
    out = args.out_dir
    caption = (f"§5.7 readout-error estimators, noise={args.noise}, "
