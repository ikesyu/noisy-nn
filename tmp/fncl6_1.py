...
|---|---|---|
| 乱数生成 | Box-Muller / CLT 近似 (乗算器・LUT・複数一様乱数) | LFSR そのまま (シフト + XOR) |
| 交差 activation | 比較器 x2 + XOR | 比較器 x2 + XOR (同一) |
| 期待応答 phi_bar(d) | 2F(1-F), F は erf (LUT / 区分近似) | 厳密な 2 次多項式 0.5*(1-((d-c)/r)^2) |
| 解析局所微分 phi_bar'(d) | 2(1-2F)p(d), erf + exp (LUT) | 一次式 -(d-c)/r^2 (減算 1 + 乗算 1) |
| 分布フリー局所微分 phi_T'(d) | xor カウンタ差分 / 2h (分布知識不要) | 同一 (分布知識不要) |
| 学習統計 (Cov, Var) | カウンタ + 積和 + 除算 | 同一 (z 二値のため大半は gated add) |

- 分布フリーの phi_T' を使う限り、ノイズ分布の解析式は forward にも backward にも
  一切現れない (§5.4/§5.5 で kde = analytic を確認)。
- 一様ノイズの利点は「解析式を使いたい場合ですら一次式で済む」+「乱数生成が
  LFSR で閉じる」の 2 点に集約される。
"""
