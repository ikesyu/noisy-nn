...
#     losses = wl["losses"]
#     switch_losses = torch.load(f"switch_learn_{seed}_{switch_loc}.pt",
#                                map_location="cpu")
#     for model_name, loss in losses.items():
#         epoch = range(0, len(loss))
#         plt.semilogy(epoch, loss, "--", label=model_name)

#     for (src_name, dst_name), loss in switch_losses.items():
#         epoch = range(switch_loc, switch_loc+len(loss))
#         plt.semilogy(epoch, loss, "--", label=f"{src_name} {dst_name}")
