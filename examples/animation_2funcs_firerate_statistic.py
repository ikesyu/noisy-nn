...
Panels (updated every animation frame):
    ...
    Lower ×2 : firing-rate map per hidden layer
               (CrossingStatistic output — T samples drawn internally,
                result is the expectation over T; output shape is [N, D])

Model: SimpleNNNStatistic (statistical estimate, default t=10 samples)

Run from the project root:

    python examples/animation_2funcs_firerate_statistic.py
"""
...
# --- Model ---
net       = model.SimpleNNNStatistic(structure=STRUCTURE)
...
# --- Forward hooks: capture hidden-layer CrossingStatistic outputs ---
# CrossingStatistic draws T samples internally and returns the expectation.
# Output shape is [N, H] (already averaged over T — no T dimension exposed).
hidden_acts: dict[int, np.ndarray] = {}
for _i in range(n_hidden):
    hidden_acts[_i] = np.zeros((N_INPUTS, H))


def make_hook(idx: int):
    def hook(module, input, output):
        hidden_acts[idx] = output.detach().cpu().numpy()   # [N, H]
    return hook
...
for i, ax_r in enumerate(ax_rasters):
    ...
    ax_r.set_title(
        f'Hidden layer {i + 1}  ({H} neurons)  —  statistical activation (T={net.t} samples)'
    )
    tick_step = max(1, H // 5)
...
fig.suptitle(
    ...
    f'(SimpleNNNStatistic, structure={STRUCTURE})',
    fontsize=10,
)
plt.tight_layout()
...
def update(frame: int):
    ...
    # Forward pass — hooks fill hidden_acts with [N, H] arrays
    with torch.no_grad():
    ...
    # Firing-rate maps: CrossingStatistic already returns [N, H] (no T axis to average)
    for i, im in enumerate(raster_ims):
        im.set_data(hidden_acts[i].T)   # [N, H] → [H, N]
...
# ani.save('animation_2funcs_firerate_statistic.mp4', writer='ffmpeg')
plt.show()
